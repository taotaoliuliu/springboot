...
var goods = {
    ...
    URL: {
        ...
        exposer: function (productUuid) {
            return '/kill/' + productUuid + '/getMiaoshaGoodsLink';
        },
        execution: function () {
            return '/kill/' +'miaosha';
        },
        queryResult: function () { //查询秒杀结果
            return '/kill/getResult';
        },
        order: function () { //下单
            return '/kill/order';
        },
    ...
    //轮训查询秒杀结果
    queryResult: function (productUuid,node){    	
		$.ajax({
			type: "post",
			...
			success: function(data, status) {
				if(status == "success" && data.code == 4) {
					//停止查询定时任务
					window.clearInterval(timerFlag); 
					
					// 下单token存起来，用于下单
					$.cookie('token', data.data, {expires: 1, path: '/'});
    ...
    detail: {
        ...
        init: function (params) {
            ...
            var productUuid = params['productUuid'];
    		...
    		$.ajax({
    			...
    			success: function(data, status) {
    				if(status == "success") {
						...
						var nowTime = data;
						goods.countDown(productUuid, nowTime, startTime, endTime,code);
    				} else {
    ...
    handlergoods: function (productUuid, node, nowTime, startTime, endTime,code) {
    	console.log(code);
    	if(code==1){
            ...
            var killUrl = goods.URL.execution();
            $('#killBtn').one('click', function () {
               ...
               // node.html("<img id='myloading' src='/static/img/loading.gif'></img")
        		...
        		$.ajax({
        			...
        			success: function(data, status) {
        				...
        				if(status == "success") {
                            ...
                            goods.queryResult(productUuid,node);
                            timerFlag = window.setInterval(function(){
                            	goods.queryResult(productUuid,node);
    		...
    		node.before('您已经获得秒杀资格，有效期：3分钟，请立即下单');
			node.html('<button class="btn btn-primary btn-lg" id="orderBtn">立即下单</button>');
    	}
...
/*		$.ajax({
    ...
    countDown: function (productUuid, nowTime, startTime, endTime,code) {
        console.log(productUuid + '_' + nowTime + '_' + startTime + '_' + endTime);
        var goodsBox = $('#goods-box');
        if (nowTime > endTime) {
            //秒杀结束
            goodsBox.html('秒杀结束!');
            ...
            goodsBox.countdown(killTime, function (event) {
                ...
                goods.handlergoods(productUuid, goodsBox, nowTime, startTime, endTime,code);
            });
        } else {
            //秒杀开始
            goods.handlergoods(productUuid, goodsBox, nowTime, startTime, endTime,code);
