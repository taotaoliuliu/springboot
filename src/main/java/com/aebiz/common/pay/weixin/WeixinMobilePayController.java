...
import com.aebiz.common.pay.weixin.utils.MobileUtil;
...
public class WeixinMobilePayController {
    ...
    public String  h5pay(Product product,ModelMap map) {
		logger.info("纯H5支付(不建议在APP端使用)1111111111");
	...
	@RequestMapping(value="h5pay2",method=RequestMethod.POST)
    public String  h5pay2(Product product,ModelMap map) {
		logger.info("纯H5支付(不建议在APP端使用)22222222222");
		//mweb_url为拉起微信支付收银台的中间页面，可通过访问该url来拉起微信客户端，完成支付,mweb_url的有效期为5分钟。
		String mweb_url =  weixinPayService.weixinPayH52(product);
		if(StringUtils.isNotBlank(mweb_url)){
			return "redirect:"+mweb_url;
		}else{
			return "redirect:https://blog.52itstyle.com";//自定义错误页面
	 ...
	 * @return
	 */
	@RequestMapping(value="h5pay3",method=RequestMethod.POST)
	@ResponseBody
    public String  h5pay3(Product product,ModelMap map,HttpServletRequest request) {
		logger.info("纯H5支付(不建议在APP端使用)22222222222");
		//mweb_url为拉起微信支付收银台的中间页面，可通过访问该url来拉起微信客户端，完成支付,mweb_url的有效期为5分钟。
		
		product.setIp(PayCommonUtil.getIpAddress(request));
		String mweb_url =  weixinPayService.weixinPayH52(product);
	...
	public String dopay(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String notify_url ="/weixinMobile/WXPayBack";//回调接口
		...
		packageParams.put("spbill_create_ip", PayCommonUtil.getIpAddress(request));// 发起人IP地址
		...
		if("SUCCESS".equals(returnCode)){
			...
			if("SUCCESS".equals(resultCode)){
				...
				finalpackage.put("appId",  WeiXinConfig.app_id);
				...
				sign = PayCommonUtil.createSign("UTF-8", finalpackage,WeiXinConfig.app_key);
				url.append("redirect:/weixinMobile/payPage?");
				url.append("timeStamp="+timestamp+"&nonceStr=" + nonceStr + "&package=" + packages);
				url.append("&signType=MD5" + "&paySign=" + sign+"&appid="+ WeiXinConfig.app_id);
