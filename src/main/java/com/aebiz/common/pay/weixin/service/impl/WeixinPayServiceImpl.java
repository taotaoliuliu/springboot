package com.aebiz.common.pay.weixin.service.impl;
...
import com.aebiz.common.pay.common.constants.Constants;
import com.aebiz.common.pay.common.model.Product;
import com.aebiz.common.pay.common.utils.CommonUtil;
import com.aebiz.common.pay.weixin.service.IWeixinPayService;
import com.aebiz.common.pay.weixin.utils.ClientCustomSSL;
import com.aebiz.common.pay.weixin.utils.HttpUtil;
import com.aebiz.common.pay.weixin.utils.PayCommonUtil;
import com.aebiz.common.pay.weixin.utils.WeiXinConfig;
import com.aebiz.common.pay.weixin.utils.XMLUtil;
import com.aebiz.common.pay.weixin.utils.ZxingUtils;
...
public class WeixinPayServiceImpl implements IWeixinPayService {
	...
	public String weixinPay2(Product product) {
		...
		try {
			...
			packageParams.put("total_fee", "100");// 总金额 单位分
			packageParams.put("spbill_create_ip", "192.168.137.66");// 发起人IP地址
			packageParams.put("notify_url", WeiXinConfig.notify_url);// 回调地址
			...
			if("SUCCESS".equals(returnCode)){
				...
				if("SUCCESS".equals(resultCode)){
					...
					WeiXinConfig.shorturl(urlCode);//转换为短链接
	...
	public void weixinPay1(Product product) {
		...
		//生成签名
		String sign = PayCommonUtil.createSign("UTF-8", packageParams, WeiXinConfig.app_key);
		//组装二维码信息(注意全角和半角：的区别 狗日的腾讯)
    	StringBuffer qrCode = new StringBuffer();
    	qrCode.append("weixin://wxpay/bizpayurl?");
    	qrCode.append("appid="+WeiXinConfig.app_id);
    	qrCode.append("&mch_id="+WeiXinConfig.mch_id);
	...
	public String weixinRefund(Product product) {
		...
		//	totalFee =  CommonUtil.subZeroAndDot(totalFee);
			...
			packageParams.put("sign", sign);// 签名
			String requestXML = PayCommonUtil.getRequestXml(packageParams);
			String weixinPost = ClientCustomSSL.doRefund(WeiXinConfig.REFUND_URL, requestXML).toString(); 
	...
	public String weixinCloseorder(Product product) {
		...
		try {
			...
			packageParams.put("sign", sign);// 签名
			String requestXML = PayCommonUtil.getRequestXml(packageParams);
			String resXml = HttpUtil.postData(WeiXinConfig.CLOSE_ORDER_URL, requestXML);
			Map map = XMLUtil.doXMLParse(resXml);
			String returnCode = (String) map.get("return_code");
			if("SUCCESS".equals(returnCode)){
				String resultCode =  (String) map.get("result_code");
	...
	public void saveBill() {
		try {
			...
			SortedMap<Object, Object> packageParams = new TreeMap<Object, Object>();
			WeiXinConfig.commonParams(packageParams);//公用部分
			...
			packageParams.put("sign", sign);// 签名
			String requestXML = PayCommonUtil.getRequestXml(packageParams);
			String resXml = HttpUtil.postData(WeiXinConfig.DOWNLOAD_BILL_URL, requestXML);
	...
	public String weixinPayMobile(Product product) {
		...
		return null;//SnsAPI.connectOauth2Authorize(WeiXinConfig.app_id, redirect_uri, true,null);
	}
	@SuppressWarnings("rawtypes")
	@Override
	public String weixinPayH5(Product product) {
		...
		try {
			// 账号信息
			String key = WeiXinConfig.app_key; // key
			key="pM4gYqiezCrIfagYauL5XrHO4NAjqAAL";
			String trade_type = "MWEB";//交易类型 H5 支付 
			SortedMap<Object, Object> packageParams = new TreeMap<Object, Object>();
			WeiXinConfig.commonParams(packageParams);
			packageParams.put("product_id", "88889993");// 商品ID
			packageParams.put("body", product.getBody());// 商品描述
			packageParams.put("out_trade_no", product.getOutTradeNo());// 商户订单号
			String totalFee = product.getTotalFee();
			totalFee =  CommonUtil.subZeroAndDot(totalFee);
			...
			packageParams.put("spbill_create_ip", "192.168.137.1");// 发起人IP地址
			packageParams.put("notify_url", "http://jstest.jslink.com/delipaycenter/payment/weixinpay/notify");// 回调地址
			packageParams.put("trade_type", trade_type);// 交易类型
			//H5支付专用 
			JSONObject value = new JSONObject();
			value.put("type", "WAP");
			value.put("wap_url", "http://jstest.jslink.com");////WAP网站URL地址
	...
	@SuppressWarnings("rawtypes")
	@Override
	public String weixinPayH52(Product product) {
		logger.info("订单号：{}发起H5支付",product.getOutTradeNo());
		String  mweb_url = "";
		try {
			// 账号信息
			...
			key="6ad668cb5dda4bdb88480592a2c130d2";
			...
			packageParams.put("appid", "wxedbcf850d158a886");// 公众账号ID
			packageParams.put("mch_id", "1399708202");// 商户号
			packageParams.put("nonce_str", nonce_str);// 随机字符串
			
			packageParams.put("product_id", "3334005");// 商品ID
			packageParams.put("body", "mi6---16g大内存");// 商品描述
			packageParams.put("out_trade_no", product.getOutTradeNo());// 商户订单号
			String totalFee = product.getTotalFee();
			//totalFee =  CommonUtil.subZeroAndDot(totalFee);
			packageParams.put("total_fee", "1");// 总金额
			//H5支付要求商户在统一下单接口中上传用户真实ip地址 spbill_create_ip
			packageParams.put("spbill_create_ip", product.getIp());// 发起人IP地址
			logger.info("ip==========="+product.getIp());

			
			packageParams.put("notify_url", "https://devyf.qwang.com.cn/smallshop/public/weixin/notify");// 回调地址
			packageParams.put("trade_type", trade_type);// 交易类型
			//H5支付专用 
			JSONObject value = new JSONObject();
			//value.put("type", "WAP");
	...
	public void orderquery(Product product) {
		try {
			...
			packageParams.put("sign", sign);// 签名
			String requestXML = PayCommonUtil.getRequestXml(packageParams);
			String resXml = HttpUtil.postData(WeiXinConfig.CHECK_ORDER_URL, requestXML);
			Map map = XMLUtil.doXMLParse(resXml);
			String returnCode = (String) map.get("return_code");
...
public static void main(String[] args) {
	String url="https://blog.csdn.net/lhwomg/article/details/53023197";
	ZxingUtils.getQRCodeImge(url, 256, "E:/aaa.png");// 生成二维码
	}
